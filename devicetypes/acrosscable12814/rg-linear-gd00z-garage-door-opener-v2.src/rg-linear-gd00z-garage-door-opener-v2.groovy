...
metadata {
    definition(name: "RG Linear GD00Z Garage Door Opener - V2", 
...
def zwaveEvent(BarrierOperatorReport cmd) {
	...
	log.debug("cmd: ${cmd}")
	log.debug("cmd.barrierState: ${cmd.barrierState}")
	...
	switch (cmd.barrierState) {
        ...
        case BarrierOperatorReport.BARRIER_STATE_UNKNOWN_POSITION_MOVING_TO_CLOSE:
            map.descriptionText = "$device.displayName is opening"
            map.value = "closing"
            log.debug("INFO: ${map.descriptionText}")
            break
...
//                result << createEvent(name:"lastBattbatteryeryStatus", value:nowString, descriptionText: map.descriptionText)
                break
            case 0x4B:
...
def refresh() {
    ...
    secureSequence([
                ...
                //,zwave.batteryV1.batteryGet() //eeddir
                //,zwave.powerlevelV1.powerlevelGet()//eeddir
                //,zwave.notificationV3.notificationGet()
                //,zwave.notificationV3.notificationSupportedGet()//eeddir
        ], 4200)
    /* */
}
