...
metadata {
	...
	definition (name: "Aeon Multisensor 6 (Advanced) v2", 
                namespace: "acrosscable12814", 
                author: "Dieter Rothhardt", 
                //ocfDeviceType: "x.com.st.d.sensor.multifunction",
                manufacturerName: "SmartThingsCommunity",
    			vid: "50cc500e-a512-3092-8a23-57ca81c7a4c1",
			    mnmn: "SmartThingsCommunity"
               ) {// //oic.d.thermostat//oic.r.temperature//, vid:"generic-motion-7"
		...
		capability "acrosscable12814.firmware"
		...
		capability "acrosscable12814.resetTamperAlert"
	...
	preferences {
		...
		input "reportInterval", "enum", title: "Report Interval", description: "How often the device should report in minutes",
			options: ["1 minute", "2 minutes", "4 minutes", "8 minutes", "15 minutes", "30 minutes", "1 hour", "6 hours", "12 hours", "18 hours", "24 hours"]
	}
...
def updated() {
    ...
    if (state.rawFW) {
        updateDataValue("firmware", "${state.rawFW}")
        sendEvent(name: "currentFirmware", value: "${state.rawFW}")
    } else if (device.currentValue("currentFirmware") != null) {
        updateDataValue("firmware", "${device.currentValue("currentFirmware")}")
        sendEvent(name: "currentFirmware", value: "${device.currentValue("currentFirmware")}")
    }
}
...
def zwaveEvent(physicalgraph.zwave.commands.versionv1.VersionReport cmd) {
    ...
    if(cmd.applicationVersion && cmd.applicationSubVersion) {
        ...
        updateDataValue("firmware", "${state.rawFW}")
        createEvent(name: "currentFirmware", value: "${state.rawFW}")
    }
}
...
def configure() {
	...
	commands(request, (state.sec || zwaveInfo?.zw?.contains("s")) ? 2000 : 500)
...
def resetTamperAlert() {
    ...
    sendEvent(name: "currentFirmware", value: "n/a")
}
