...
metadata {
    definition (name: "Rachio Zone V2", namespace: "acrosscable12814", author: "Dieter Rothhardt", vid: "4151a426-39fc-3a22-aefa-dbc9762fc622", mnmn: "SmartThingsCommunity" ) {
        capability "Refresh"
//        capability "Switch"
//        capability "Actuator"
        capability "Valve"
//        capability "Sensor"
        ...
        capability "acrosscable12814.rachioScheduleType"
        capability "acrosscable12814.rachioNozzleType"
        capability "acrosscable12814.rachioSoilType"
        capability "acrosscable12814.rachioSlope"
        capability "acrosscable12814.rachioShade"
        capability "acrosscable12814.rachioCrop"
        capability "acrosscable12814.rachioZoneArea"
        capability "acrosscable12814.rachiozonename"
...
def setRachioTriggerManualZone(value) {
	...
	sendEvent(name: 'rachioTriggerManualZone', value: "Run Zone Manually")
	startZone()
}
...
def initialize() {
    ...
    sendEvent(name: "rachioZoneName", value: "n/a")
    sendEvent(name: "rachioScheduleType", value: "n/a")
	sendEvent(name: "rachioNozzleType", value: "n/a")
	sendEvent(name: "rachioSoilType", value: "n/a")
	sendEvent(name: "rachioSlope", value: "n/a")
	sendEvent(name: "rachioShade", value: "n/a")
    sendEvent(name: "rachioCrop", value: "n/a")
	sendEvent(name: "rachioZoneArea", value: "n/a")
	sendEvent(name: "rachioSetManualZoneRuntime", value: "0", unit: "min")
	sendEvent(name: "rachioTriggerManualZone", value: "0 min", unit: "min")
    verifyDataAttr()
}
...
def isWateringEvent(status, zoneId) {
    ...
    def newState2 = isOn ? "Watering" : "Idle"
    if(isStateChange(device, "watering", newState.toString())) {
        ...
        sendEvent(name: "rachioWateringStatus", value: newState2)
...
def zoneSquareFeetEvent(val) {
    ...
    log.debug("rachioZoneArea: ${newState}")
        sendEvent(name: "rachioZoneArea", value: "${newState} sq ft")
}
...
def zoneNameEvent(val) {
    ...
    log.debug("rachioZoneName: ${newState}")
    	sendEvent(name: "rachioZoneName", value: newState)
}
...
def setZoneWaterTime(timeVal) {
    ...
    if(isStateChange(device, "zoneWaterTime", newVal.toString())) {
        ...
        sendEvent(name: 'rachioTriggerManualZone', value: "Run This Zone\n${newVal} Minute(s)")
    }
}
...
def scheduleDataEvent(data) {
    ...
    if(isStateChange(device, "scheduleType", curSchedType?.toString().toLowerCase())) {
        ...
        sendEvent(name: "rachioScheduleType", value: curSchedType?.toString().toLowerCase()?.capitalize())
    }
...
def customNozzleDataEvent(data) {
    ...
    if(data && data?.name && isStateChange(device, "nozzleName", fmtString(data?.name).toString())) {
        ...
        sendEvent(name: "rachioNozzleType", value: fmtString(data?.name).toString())
}
...
def customSoilDataEvent(data) {
    ...
    if(data && data?.name && isStateChange(device, "soilName", fmtString(data?.name).toString())) {
        ...
        sendEvent(name: "rachioSoilType", value: fmtString(data?.name).toString())
}
...
def customSlopeDataEvent(data) {
    ...
    if(slope && isStateChange(device, "slopeName", slope.toString())) {
        ...
        sendEvent(name: "rachioSlope", value: fmtString(data?.name).toString())
}
...
def customCropDataEvent(data) {
    ...
    if(data && data?.name && isStateChange(device, "cropName", fmtString(data?.name).toString())) {
        ...
        sendEvent(name: "rachioCrop", value: fmtString(data?.name).toString())
}
...
def customShadeDataEvent(data) {
    ...
    if(data && data?.name && isStateChange(device, "shadeName", fmtString(data?.name).toString())) {
        ...
        sendEvent(name:'rachioShade', value: fmtString(data?.name).toString())
}
...
//eeddir
        ...
        sendEvent(name:'rachioWateringStatus', value: "Watering")
    }
    else {
        markOffLine()
    }
